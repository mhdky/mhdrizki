        ...
        <div class="w-full bg-[#364352] flex justify-center">
            ...
            <div class="w-[1200px] flex">
                ...
                <div class="w-full pb-[100vh] bg-body">
                    {{-- name section --}}
                    <h1 class="text-black ml-[27%] my-5 text-3xl font-semibold w-max border-b-4 border-orange-theme">Message</h1>
                        ...
                        @foreach ($contacts as $contact)
                            <tr class="bg-[#E4E4E4] {{ $loop->iteration % 2 == 1 ? 'bg-[#E4E4E4]' : 'bg-[#d4d4d4]' }}">
                                ...
                                <td class="email-table text-center capitalize py-2 border-2 border-body relative">
                                    {{ Str::limit($contact->name_c, 11) }}
                                    <p class="email-detail w-[110px] max-h-8 hidden overflow-auto absolute right-1/2 translate-x-1/2 -bottom-5 bg-[#000000ab] p-1 text-body text-[11px] rounded-md">{{ $contact->name_c }}</p>
                                </td>
                                <td class="email-table p-2 text-center text-[11px] border-2 border-body relative">
                                    {{ Str::limit($contact->email_c, 20) }}
                                    <p class="email-detail w-[110px] max-h-8 hidden overflow-auto absolute right-1/2 translate-x-1/2 -bottom-5 bg-[#000000ab] p-1 text-body text-[11px] rounded-md">{{ $contact->email_c }}</p>
                                </td>
                                <td class=" text-center py-2 text-[0.7rem] border-2 border-body">{{ $contact->created_at->diffForHumans() }}</td>
                                <td class="email-table border-2 p-2 border-body relative">
                                    {{ Str::limit($contact->message_c, 50) }}
                                    <p class="email-detail w-[110px] max-h-8 hidden overflow-auto absolute right-1/2 translate-x-1/2 -bottom-5 bg-[#000000ab] p-1 text-body text-[11px] rounded-md">{{ $contact->message_c }}</p>
                                </td>
